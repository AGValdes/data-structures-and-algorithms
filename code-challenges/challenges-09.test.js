...
const mapCurrentEvents = () => {
  // Solution code here...
  let newEventsArray = currentEvents.news.map(item => {
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  let indexPlusOne = arr.reduce((prev, cur, inx) => {
    return inx + 1;
  });
  return indexPlusOne;
};
...
const returnNames = (arr) => {
  ...
  arr.reduce((accumulator, person, idx) => {
    return namesArray.push(person.name);
  }, 0);
  return namesArray;
};
...
const reversedString = (str) => {
  ...
  let newString = array.reduce((reversed, character) => {
