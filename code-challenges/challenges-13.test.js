...
const firstLetters = (arr) => {
  // Solution code here...
  let newArray = arr.map(string => string.charAt(0));
  return newArray;
...
const standardizePhoneNumbers = (arr) => {
  ...
  let newArray = [];
  arr.forEach(number => {
    let firstSection = number.substr(1, 3);
    let secondSection = number.substr(6, 3);
    let thirdSection = number.substr(10, 13);
    console.log(firstSection, secondSection, thirdSection);
    let newNumber = `${firstSection}${secondSection}${thirdSection}`;
