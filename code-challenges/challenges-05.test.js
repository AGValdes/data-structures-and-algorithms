...
const templateWithJQuery = () => {
  ...
  starWarsPeople.forEach((item) => {
    ...
    $templateClone.html($template);


    $templateClone.find('h2').text(item.name);
    $templateClone.find('h3').text(item.height);
    $templateClone.find('p').text(item.eye_color);
...
const howMuchPencil = (str) => {
  ...
  for (let i = 0; i < str.length; i++) {
    let newWord = str.slice(0, str.length);
...
const wordsToCharList = (arr) => {
  // Solution code here...
  let seperator = /\.\./;
  let newArr = arr.split(seperator);
  return newArr;
};
