...
const createServer = () => {
  ...
  app.get('/aboutme', (request, response) => {
    response.send('I like video games and unicorns and most importantly, video games ABOUT unicorns');
  });

  app.get('/favoritefoods', (request, response) => {
    response.send(['sushi', 'pizza', 'slim jims', 'ramen noodles']);
  });
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  //   let unPackedArray = input.reduce((prev, curr) => {

  //     prev.push(curr.reduce((prev, curr) => {
  //       if (curr % 5 === 0 && typeof curr === 'number') {
  ...
  //     // return prev;
  //   }, []);
  //   console.log('unpackedarray', unPackedArray);
  //   let newestArray = unPackedArray.map(val => {
