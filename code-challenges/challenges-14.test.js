...
const createServer = () => {
  ...
  app.get('/', statusTwoHundred);
  app.delete('/things/1', statusFourOFive);

  function statusTwoHundred(req, res) {
    res.status(200).send('it worked!');
  }

  function statusFourOFive(req, res) {
    res.status(405).send('sorry did not work');
  }
...
const toTitleCase = (arr) => {
  ...
  let newArray = arr.map(item => {
    return (`${item.charAt(0).toUpperCase()}${item.substr(1, 10)}`);
  });
  return newArray;
};
